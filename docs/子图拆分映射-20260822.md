...
| `torch.ops.aten.{add,mul,sub,div}.Tensor` | `_rule_elementwise_binary` |
| `torch.ops.aten.mean.dim` | `_rule_reduce_last` |
| `torch.ops.aten.{tanh,pow,rsqrt,neg,silu,relu,sigmoid,exp,sqrt,reciprocal}` | `_rule_unary` |

这个表必须和 `graph.partition` 的分区口径保持一致。否则问题 1 认为某个算子可落 DPU，但问题 2 不知道怎么传播它的布局。

**口径是黑名单**（`graph.partition.HOST_ONLY`，2026-09-22 起）：默认下设备，只有
列进黑名单的留主机。反过来做（白名单）会让每个没想到的算子静默落主机——图照样
跑完、数值也对，只是那些算子根本没在设备上执行。黑名单下漏一个新算子会在
`propagate_specs` 这儿直接抛错，吵闹但看得见。

所以两张表的同步方向也反了：**新增算子不需要动 `HOST_ONLY`，但必须补 `RULE_TABLE`
和 `runtime/kernels.py`**；要让一个算子留主机，得显式列进 `HOST_ONLY` 并写明理由
（见那里的五类：注意力有专用 KV 处理函数、命令编码表达不了张量列表/子区间、
纯布局算子、编译脚手架、目标模型里不出现的算子）。

`tests/test_partition.py::test_every_device_op_has_a_shard_rule_and_a_kernel` 按
真实导出的 llama 图锁住这个闭合条件。
