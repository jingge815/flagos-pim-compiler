...
> **已于 20260905 修复**，详见 `docs/genesimsupporTp-20260905.md`。
>
> 修复要点：放置导出改为记录全部分片（`shards` 列表 + 行列切标注），GeneSim 侧把一个
> 多分片算子在图上展开成每台 DPU 一个独立算子，行切之后插入真实的归约节点。本节末尾
> 说的「要修得改 sidecar 的表达能力，让一个算子能带一组 dpu_id，GeneSim 侧也要能把一个
> 算子摊到多个 PU 上」正是修复的实际做法。
>
> 修复后三种策略都端到端实测过：下表「失真情况」一列已全部消除，`tp8_pp1` 现在是
> 8 台各承担 224 个分片（不再全部标到 dpu0），`tp2_pp4` 是 8 台各 56 个（不再只有
> dpu 0/2/4/6）。
>
> **仍然近似的部分**：归约的通信量准确，但耗时按点对点传输计算，未建模环形/树形归约的
> 优化与带宽竞争。所以绝对耗时适合同类配置间的相对比较，不宜当精确预测。
>
> 以下保留原始记录，作为问题现场的存档。
...
> **20260905 补注：部分改善，本条未完全解决。**
>
> 上一条修复之后，一个切分组的 N 个分片现在会落到 **N 个不同的 DPU**（对应 N 个不同的
> PIMVPU），跨分片的传输延迟因此是真实计算的——这一点已由测试断言「不同 DPU 必须落到
> 不同 VPU」守住，否则跨分片搬运会被算成零耗时。
>
> 但本条说的「同一个 DPU 内部的多个 TensorPU 协同算一个算子」仍然没有建模：一个分片
> 落到它那台 DPU 的一个 PU 上，同 cluster 的其余 PU 依然空闲。这是另一个层次的并行
> （设备内 vs 设备间），需要放置桥接能表达「一个算子用哪几个 PU」，本次没有涉及。
