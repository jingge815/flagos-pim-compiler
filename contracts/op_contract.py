...
字段是三个：`op`、`arg_shapes`、`dtype`。不需要更多——`linear` 白名单实现
（`graph/spec_prop.py` 拒绝 tensor bias、`runtime/kernels.py` 无 bias add）不需要
bias；每个 DPU 收到的已经是切分传播后的本地 shape，算子编译器不需要知道全局切分
方式；MRAM 内的字节 offset 是运行期值（KV cache 增长会变），本就不该进编译期
契约，运行期经 `cmd.reads`/`cmd.writes` 传给 kernel；WRAM 在这个仓库的数值执行
侧没有真实内存建模（`backend/dpu_sdk.py` 只有一块连续 MRAM），wram 相关字段没有
代码会消费。
