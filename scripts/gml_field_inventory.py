...
def normalize(key: str) -> str:
    ...
    要留下。区别在于槽位号只取 0/1 且属于上面那张表里的字段族。
    """
    # 先剥掉最后一段数字，看剩下的是不是一个带槽位后缀的族。
    stripped = re.sub(r"_\d+$", "", key)
    slot = re.match(r"^(.*)_([01])$", stripped)
    if slot and slot.group(1) in SLOT_SUFFIXED:
        return stripped
    if stripped in SLOT_SUFFIXED and re.match(r"^.*_([01])$", key):
        # `fpsu_mode_0` 本身就是终态，剥数字反而丢了槽位号。
        return key
    return stripped
