...
AI 辅助开发最容易堆出大量重复、投机、永不执行的代码。以下是硬约束。
...
- **算子融合是例外，现在就做**：GML 把激活折进主算子的 `contraction` 块，融合是目标格式的硬要求而非优化选项，所以它属于当前阶段。只做"主算子 + 激活/池化"这一种融合，不做通用融合框架。
...
- 命名跟方案术语一致：`part_id`、`dpu_id`、`placement`、`valid_len`——不另起别名。
